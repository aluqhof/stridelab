...
interface PRProgression {
  distance: string;
  history: { date: string; time: number; activityId: number; activityName: string }[];
}
...
export async function GET(request: Request) {
  try {
    ...
    const runActivities = activities.filter(
      (a) => (a.sport_type === "Run" || a.sport_type === "VirtualRun") && a.distance > 0
    );
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const totalElevation = weekActivities.reduce((sum, a) => sum + a.total_elevation_gain, 0);
      ...
      const avgPace = weekRuns.length > 0 && totalDistance > 0
        ? weekRuns.reduce((sum, a) => sum + a.moving_time, 0) / (weekRuns.reduce((sum, a) => sum + a.distance, 0) / 1000)
        : 0;
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const totalElevation = monthActivities.reduce((sum, a) => sum + a.total_elevation_gain, 0);

      const avgPace = monthRuns.length > 0
        ? monthRuns.reduce((sum, a) => sum + a.moving_time, 0) / (monthRuns.reduce((sum, a) => sum + a.distance, 0) / 1000)
        : 0;
    ...
    const thisYearActivities = activities.filter((a) => new Date(a.start_date_local) >= thisYearStart);
    const lastYearActivities = activities.filter((a) => {
      const date = new Date(a.start_date_local);
    ...
    const yearComparison = {
      thisYear: {
        distance: thisYearActivities.reduce((sum, a) => sum + a.distance, 0),
        time: thisYearActivities.reduce((sum, a) => sum + a.moving_time, 0),
        activities: thisYearActivities.length,
        elevation: thisYearActivities.reduce((sum, a) => sum + a.total_elevation_gain, 0),
      },
      lastYear: {
        distance: lastYearActivities.reduce((sum, a) => sum + a.distance, 0),
        time: lastYearActivities.reduce((sum, a) => sum + a.moving_time, 0),
        activities: lastYearActivities.length,
        elevation: lastYearActivities.reduce((sum, a) => sum + a.total_elevation_gain, 0),
      },
    };
    ...
    // Estimate max HR
    const maxHRActivities = activities.filter((a) => a.max_heartrate && a.max_heartrate > 150);
    const estimatedMaxHR = maxHRActivities.length > 0
    ...
    const timeOfDayMap = new Map<number, { count: number; totalPace: number; totalHR: number; hrCount: number }>();

    for (const act of runActivities) {
      const hour = getHours(new Date(act.start_date_local));
      const existing = timeOfDayMap.get(hour) || { count: 0, totalPace: 0, totalHR: 0, hrCount: 0 };
    ...
    const dayOfWeekMap = new Map<number, { count: number; distance: number; totalPace: number }>();

    for (const act of activities) {
      const day = getDay(new Date(act.start_date_local));
      const existing = dayOfWeekMap.get(day) || { count: 0, distance: 0, totalPace: 0 };
    ...
    const dayOfWeekData: DayOfWeekData[] = [1, 2, 3, 4, 5, 6, 0].map((day) => {
      const data = dayOfWeekMap.get(day) || { count: 0, distance: 0, totalPace: 0 };
      return {
        day,
    ...
    const paceHRData: PaceHRPoint[] = runActivities
      .filter((a) => a.average_heartrate && a.average_heartrate > 100 && a.distance > 3000)
      .slice(0, 50)
      .map((a) => {
    ...
    const avgRecentEff = recentEfficiency.length > 0
      ? recentEfficiency.reduce((sum, p) => sum + p.efficiency, 0) / recentEfficiency.length
      : 0;
    const avgOlderEff = olderEfficiency.length > 0
      ? olderEfficiency.reduce((sum, p) => sum + p.efficiency, 0) / olderEfficiency.length
      : 0;

    const efficiencyTrend = avgOlderEff > 0
      ? ((avgRecentEff - avgOlderEff) / avgOlderEff) * 100
      : 0;
    ...
    const avgWeeklyDistance = weeklyData.slice(-4).reduce((sum, w) => sum + w.distance, 0) / 4;
    const avgWeeklyTime = weeklyData.slice(-4).reduce((sum, w) => sum + w.time, 0) / 4;
    const avgWeeklyActivities = weeklyData.slice(-4).reduce((sum, w) => sum + w.activities, 0) / 4;
    ...
    // ============ 80/20 RULE CHECK ============
    const totalEasyTime = zoneDistribution.reduce((sum, w) => sum + w.zone1 + w.zone2, 0);
    const totalHardTime = zoneDistribution.reduce((sum, w) => sum + w.zone4 + w.zone5, 0);
    const totalZoneTime = zoneDistribution.reduce(
      (sum, w) => sum + w.zone1 + w.zone2 + w.zone3 + w.zone4 + w.zone5, 0
    );

    const easyPercent = totalZoneTime > 0 ? (totalEasyTime / totalZoneTime) * 100 : 0;
    const hardPercent = totalZoneTime > 0 ? (totalHardTime / totalZoneTime) * 100 : 0;
